...
llms_result <- key_metrics |>
  filter(.data$metric == "Observed /llms.txt among resolved observations")
...
llms_presence_data <- tibble::tibble(
  ...
  mutate(
    proportion = .data$count / llms_result$denominator,
    label = paste0(
      percent(.data$proportion, accuracy = 0.1),
      "\n",
      comma(.data$count)
    ),
    label_color = if_else(.data$status == "Observed", "white", "grey20")
  )

llms_presence_plot <- ggplot(
  llms_presence_data,
  aes(x = "", y = .data$proportion, fill = .data$status)
) +
...
restriction_source_data <- restriction_source_summary |>
  mutate(
    ...
    label_hjust = if_else(.data$proportion > 0.75, 1.04, -0.04),
    label_color = if_else(.data$proportion > 0.75, "white", "grey20")
  )

restriction_source_plot <- ggplot(
  restriction_source_data,
  aes(x = .data$proportion, y = .data$source)
) +
...
content_plot_data <- content_summary |>
  filter(.data$signal %in% c("Yes", "No")) |>
  mutate(label = percent(.data$resolved_proportion, accuracy = 0.1))

content_labels <- content_plot_data |>
  filter(.data$count > 0)

unspecified_range <- content_summary |>
  filter(.data$signal == "Unspecified") |>
